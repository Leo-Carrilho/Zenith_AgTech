...
communicator.on("IsIeModeTab", (message, responseFunction) => {
    try {
        var tabId = message.arg0;
        chrome.tabs.executeScript(tabId, { code: "var _a_a_12_123 = 1;" }, () => responseFunction({ isIeModeTab: IsRunningInIEMode() }));
    }
    catch (e) {
        responseFunction({ err: 'Error ' + e });
    }
...
function IsRunningInIEMode() {
    return chrome.runtime.lastError != undefined && chrome.runtime.lastError.message == "This is not supported on tabs in Internet Explorer Mode.";
}
